...
const JAVASCRIPT_CONTROL_PROMPT = [
	...
	"",
	"The `javascript` tool has two input modes. Batch independent routine reads, searches, shell commands, and exact writes through the `actions` input; one call can contain up to eight flat actions. Use `code` for computation, branching, dependent operations, prepared JavaScript skills, and persistent notebook state. Prefer a structured `shell` action over `$` or `sh()` when no JavaScript composition is needed.",
	"",
