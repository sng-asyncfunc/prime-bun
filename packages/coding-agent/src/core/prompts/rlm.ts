...
const JAVASCRIPT_CONTROL_PROMPT = [
	...
	"",
	"Run shell commands with `await sh(command)` or Bun Shell tagged templates. `sh` uses the configured project shell and command prefix. Use ordinary JavaScript around those calls for branching, parallelism, and output processing.",
	"",
	...
	"",
	"JavaScript state persists across cells: named variables, helper functions, classes, dynamically imported modules, notes, parsed outputs, and helper data structures remain available in later turns. Top-level `await` works. Static `import` declarations are not supported in cells; use `await import('package')`.",
	"",
