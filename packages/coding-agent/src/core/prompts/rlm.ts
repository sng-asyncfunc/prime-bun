...
const JAVASCRIPT_CONTROL_PROMPT = [
	...
	"",
	'When writing text containing backticks or Markdown fences, do not wrap the whole document in one template literal; build it as an array of ordinary quoted lines joined with "\\n" so you preserve the exact file in one cell.',
...
export function buildRlmPrompt(options: RlmPromptOptions): string {
	...
	if (installedSkills.length > 0) {
		...
		if (hasJavaScript && installedSkills.includes("edit")) {
			skillLines.push(
				"For targeted existing-file edits, use `await edit({ path: 'pkg/file.ts', oldStr, newStr })` with exact old/new strings built from inspected content.",
			);
		}
	}
