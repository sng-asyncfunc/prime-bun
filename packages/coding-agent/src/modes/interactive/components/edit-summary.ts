...
import type { JavaScriptToolDetails } from "../../../core/tools/javascript.js";
...
export function getToolFileChanges(
	...
	if (toolName === "javascript") {
		for (const display of (result.details as JavaScriptToolDetails | undefined)?.diffs ?? []) {
...
export function formatFileChangeSummaryLine(
	...
	return truncateToWidth(`${prefix}${theme.fg("muted", path)}${counts}${hint}`, safeWidth, "");
}
