...
describe("ACP session event mapping", () => {
	...
	it("treats JavaScript as an execute tool call carrying its cell source", () => {
		expect(acpToolKind("javascript")).toBe("execute");
		...
		expect(updates).toEqual([
			{
				...
				kind: "execute",
				status: "in_progress",
				rawInput: { code: "console.log(1)" },
			},
		]);
	});

	it("carries rich JavaScript output from the fields the tool actually reports", () => {
		// The JavaScript tool reports media/diffs under `details`, so the mapping must
	...
	it("omits JavaScript rich metadata when the cell produced none", () => {
		const updates = acpUpdatesForSessionEvent({
			type: "tool_execution_end",
			toolCallId: "call-3",
			toolName: "javascript",
