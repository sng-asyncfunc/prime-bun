...
describe("ACP mode over a real Bun notebook", () => {
	...
	it("keeps JavaScript state across cells and represents each cell as an ACP execute call", {
		tags: ["kernel-heavy"],
		timeout: 180_000,
	}, async () => {
	...
	it("runs continual-harness CRUD in Bun and represents the result over ACP", {
		tags: ["kernel-heavy"],
		timeout: 180_000,
	}, async () => {
...
`);
	...
	it("exposes RLM depth and subagent APIs behind the ACP front end", {
		tags: ["kernel-heavy"],
		timeout: 180_000,
	}, async () => {
		provisioner = new BunKernelProvisioner(tempDir, {
...
`);
	...
	it("sends an agent-to-agent message from Bun and surfaces it over ACP", {
		tags: ["kernel-heavy"],
		timeout: 180_000,
	}, async () => {
		provisioner = new BunKernelProvisioner(tempDir, {
