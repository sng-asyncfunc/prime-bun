...
describe("ExtensionSelectorComponent", () => {
	...
	it("renders a multiline prompt with compact option rows", () => {
		const selector = new ExtensionSelectorComponent(
			[
				"Interrupted JavaScript cell is still running",
				"Ctrl+C sent an interrupt, but the previous cell has not stopped yet.",
				"Waiting preserves the current kernel state. Killing restarts JavaScript and loses in-memory variables.",
			].join("\n"),
		...
		expect(output).toContain("Interrupted JavaScript cell is still running");
		...
		expectTrailingBackground(
			lines.find((line) => line.includes("Interrupted JavaScript cell is still running")),
			"Interrupted JavaScript cell is still running",
		);
