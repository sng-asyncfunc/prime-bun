...
function renderCell(state: ConstructorParameters<typeof JavaScriptCellComponent>[0]): string {
	return stripAnsi(new JavaScriptCellComponent(state).render(80).join("\n"));
}

describe("JavaScriptCellComponent diff rendering", () => {
	beforeAll(() => {
		initTheme("dark");
	});
	...
	it("renders a diff with absolute line numbers and suppresses the Edited confirmation", () => {
		const out = renderCell({
			code: 'await edit({ path: "sample.ts", oldStr: "gamma", newStr: "GAMMA" })',
			details: {
				status: "ok",
				durationMs: 12,
		...
		// The redundant "Edited sample.ts" confirmation must not render as its own line.
	...
	it("coalesces multiple edits to one file into a single block with hunk separators", () => {
		const out = renderCell({
			...
			details: {
				status: "ok",
				diffs: [
					{ path: "app.ts", oldStr: "a = 1", newStr: "a = 2", startLine: 1 },
					{ path: "app.ts", oldStr: "b = 1", newStr: "b = 2", startLine: 50 },
					{ path: "app.ts", oldStr: "c = 1", newStr: "c = 2", startLine: 90 },
				],
			},
			executionStarted: true,
			argsComplete: true,
	...
	it("keeps the top line stable when expanded — only the hint flips, no header line, no layout shift", () => {
		...
		const collapsed = new JavaScriptCellComponent({ ...state, expanded: false }).render(80);
		const expanded = new JavaScriptCellComponent({ ...state, expanded: true }).render(80);
		...
		expect(stripped.filter((l) => /javascript · done/.test(l)).length).toBe(0);
	...
	it("does not show a 'no output' line under an edit-only diff", () => {
		const out = renderCell({
			code: 'await edit({ path: "a.ts", oldStr: "x", newStr: "X" })',
			details: { status: "ok", diffs: [{ path: "a.ts", oldStr: "x", newStr: "X", startLine: 1 }] },
			executionStarted: true,
			argsComplete: true,
			expanded: true,
