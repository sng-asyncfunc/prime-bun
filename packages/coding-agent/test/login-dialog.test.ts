...
describe("LoginDialogComponent", () => {
	...
	it("keeps printable copy keys available to a visible manual input", async () => {
		const dialog = new LoginDialogComponent(createFakeTui(), "anthropic", () => {}, "Anthropic");
		const url = "https://example.com/oauth";

		dialog.showAuth(url);
		const input = dialog.showManualInput("Paste the callback URL");
		dialog.handleInput("c");
		expect(mocks.copyToClipboard).not.toHaveBeenCalled();
		dialog.handleInput("\r");
		await expect(input).resolves.toBe("c");
