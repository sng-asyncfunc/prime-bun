...
/** Faux JavaScript tool that services goal.* host requests like the real Bun kernel bridge. */
function createFauxJavaScriptTool(sessionRef: { current?: AgentSession }) {
	return {
		name: "javascript",
		label: "javascript",
		description: "Execute JavaScript code in the agent kernel.",
...
describe("compaction continuation", () => {
	...
	it("e2e: an active goal keeps continuing after a successful threshold compaction", async () => {
		...
		const harness = await createHarness({
			tools: [createFauxJavaScriptTool(sessionRef)],
		...
		harness.setResponses([
			...
			fauxAssistantMessage(fauxToolCall("javascript", { code: "goal.complete" }), { stopReason: "toolUse" }),
