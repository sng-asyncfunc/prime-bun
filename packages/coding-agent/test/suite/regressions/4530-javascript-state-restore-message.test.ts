...
import {
	...
	type JavaScriptStateRestoredDetails,
...
type StateRestoreHost = {
	_onJavaScriptStateRestored(result: RestoreResult): void;
};
...
describe("ENG-4530 JavaScript state restore message", () => {
	...
	it("preserves restore context as a custom message when the next prompt is queued", async () => {
		...
		await toolStarted;
		(harness.session as unknown as StateRestoreHost)._onJavaScriptStateRestored({
			restored: ["alpha", "beta"],
			failed: [],
			path: "/tmp/kernel-state.bin",
		});
		...
		const restoreMessage = harness.session.messages.find(
			(message): message is CustomMessage =>
				message.role === "custom" && message.customType === JAVASCRIPT_STATE_RESTORED_CUSTOM_TYPE,
		);
		if (!restoreMessage || !isInjectedPromptMessage(restoreMessage)) {
			throw new Error("Expected an injected JavaScript restore message");
		}

		const component = new InjectedPromptMessageComponent(restoreMessage);
		expect(render(component)).toContain("◆ Restored JavaScript state");
		expect(render(component)).not.toContain("alpha");
		component.setExpanded(true);
		expect(render(component)).toContain("◆ Restored JavaScript state");
		expect(render(component)).not.toContain("javascript_state_restored");
		expect(render(component)).not.toContain("alpha");
	});
	...
	it("retries only undelivered input after partial scheduler delivery", async () => {
		...
		expect(harness.session.messages).toEqual([
			expect.objectContaining({ customType: JAVASCRIPT_STATE_RESTORED_CUSTOM_TYPE }),
		]);
	});

	it("shows an accurate fixed label when restoration starts a fresh kernel", () => {
		const message: CustomMessage<JavaScriptStateRestoredDetails> = {
			role: "custom",
			customType: JAVASCRIPT_STATE_RESTORED_CUSTOM_TYPE,
		...
		expect(render(component)).toContain("◆ Started fresh Bun worker");
		component.setExpanded(true);
